...
class Generator:
    ...
    def generate(self, prompt):
        # Patch: Ensure input is [batch=1, seq_len, embed_dim]
        context_emb = prompt.context_embs[0]
        if context_emb.dim() == 2:
            context_emb = context_emb.unsqueeze(0)  # [1, seq_len, dim]

        seq_len = context_emb.size(1)

        # Create fake input_ids with correct seq_len
        input_ids = torch.ones((1, seq_len), dtype=torch.long).to(self.device)

        outputs = self.model.llama_model.generate(
            inputs_embeds=context_emb,
            input_ids=input_ids,  # <<<<<< CORRECTED
        ...
        if output_token[0] == 0:  # remove <unk> token if present
