...
class Prompt:
    ...
    def generate_img_embedding(self, img_prompts):
        ...
        for items in img_prompts:
            embs = []
            for img in items:
                feats, _ = self.model.encode_img(img.unsqueeze(0))  # ✅ add batch dimension
                feats = feats.to(self.device)
                feats = feats.squeeze(0)  # ✅ remove batch dimension back
                embs.append(feats)
    ...
    def generate_context_embedding(self, batch_text_embs, batch_img_embs):
        ...
        for i in range(batch_size):
            ...
            if num_text_segs == 0 and num_img_segs == 0:  # empty context
                mixed_embs = torch.zeros(1, 0, self.model.llama_model.model.embed_tokens.weight.shape[1],
                                         device=self.device)
            else:
                s = t = 0
                mixed_embs = []
                ...
                # 🛠 Force all tensors to be 3D
                mixed_embs = [emb.unsqueeze(0) if emb.dim() == 2 else emb for emb in mixed_embs]
